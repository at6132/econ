...
from realm.population.employment import cancel_job_opening, post_job_opening
...
def _claim_player_plot(w: object) -> PlotId:
    ...
    w.ledger.transfer(
        ...
        credit=party_cash_account(human),
...
def _claim_plot_reachable_for_hiring(w: object) -> PlotId:
    """Claim a plot near a town center so an unemployed laborer can apply."""
    from realm.core.ledger import party_cash_account, system_reserve_account
    ...
    for pid, pl in w.plots.items():
        if pl.owner is not None:
            continue
        if "water" in str(pl.terrain).lower():
            continue
        if max(abs(pl.x - center.x), abs(pl.y - center.y)) > 8:
            continue
        assert claim_plot(w, human, pid)["ok"] is True
        return pid
    raise RuntimeError("no unclaimed plot near a town center")


def test_post_job_opening_ok() -> None:
    w = bootstrap_genesis(seed=501, settler_count=5)
...
def test_post_job_opening_requires_plot_ownership() -> None:
    w = bootstrap_genesis(seed=502, settler_count=5)
    pid = _claim_player_plot(w)
...
def test_cancel_job_opening() -> None:
    w = bootstrap_genesis(seed=503, settler_count=5)
    pid = _claim_player_plot(w)
